...
#%%  #The limit state as a function of the scaling factors: #For simply supported beam
    #The limit state as a function of the scaling factors: #For simply supported beam
    gX = lambda x,scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS: sceR*x[3]*(scrho_l*x[1]*scfy*x[0]*(1-0.5*scrho_l*x[1]*scfy*x[0]/(alpha_cc*scfc*x[2]))*b*d**2)-sceS*x[7]*(scGs*x[4]+scGp*x[5]+scQ*x[6])*L**2/8
    ...
    U = [x2u(fy),x2u(rho_l),x2u(fc),x2u(eR),x2u(Gs),x2u(Gp),x2u(Q),x2u(eS)] 
    
    #%% We can now write our LSF as a function of u:
    
    
    #LSF as a function of u, scX:
    gU = lambda u,scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS: gX([U[0](u[0]),
                                          U[1](u[1]),
                                          U[2](u[2]),U[3](u[3]),U[4](u[4]),U[5](u[5]),U[6](u[6]),U[7](u[7])],
                                                         scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS) 
    ...
    def alpha_next(u,scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS):
        dgdu = np.zeros(8)
        
        #Resistance
        dgdu[0] = (sceR*U[3](u[3])*scrho_l*U[1](u[1])*scfy*dxdufun(fy)(u[0])*(1-scrho_l*U[1](u[1])*scfy*U[0](u[0])/(alpha_cc*scfc*U[2](u[2])))*b*d**2) #fy
        dgdu[1] = (sceR*U[3](u[3])*scfy*U[0](u[0])*scrho_l*dxdufun(rho_l)(u[1])*(1-scrho_l*U[1](u[1])*scfy*U[0](u[0])/(alpha_cc*scfc*U[2](u[2])))*b*d**2) #rho_l
        dgdu[2] = (sceR*U[3](u[3])*scrho_l*U[1](u[1])*scfy*U[0](u[0])*scfc*dxdufun(fc)(u[2])*(0.5*scrho_l*U[1](u[1])*scfy*U[0](u[0])/(alpha_cc*scfc**2*(U[2](u[2])))**2)*b*d**2) #fc
        dgdu[3] = (scfy*U[0](u[0])*scrho_l*U[1](u[1])*sceR*dxdufun(eR)(u[3])*(1-0.5*scrho_l*U[1](u[1])*scfy*U[0](u[0])/(alpha_cc*scfc*U[2](u[2])))*b*d**2) #eR
   ...
   #Løyser g(u)=0 med hensynn til beta, basert på forrige beta og gamma
    def beta_next(alpha, beta_prev,scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS):
        equation = lambda beta: gU(beta*alpha,scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS) #this defines the fsolve should work with.
    ...
    def FORM_tol(scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS,alpha_start, beta_start, tol=1e-5, itreturn=False):
        alphas = [alpha_start]
        beta1 = beta_next(alpha_start, beta_start, scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS)
        ...
        while abs((betas[-1] - betas[-2])/betas[-1]) >= tol:
            alpha_new = alpha_next(betas[-1]*alphas[-1],scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS) #calculates new, improved alpha, using the last calculated values (by using [-1])
            alphas.append(alpha_new)                         #adds the new, improved alpha to the list of alphas
            beta_new = beta_next(alphas[-1],betas[-1],scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceS)   #calculates new, improved beta
   ...
   #Monte Carlo simulation 
    ...
    def MCS(scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceSS,n_sim):
        ...
        # LSF for MCS:
        g=sceR*eR['realisations']*(scrho_l*rho_l['realisations']*scfy*fy['realisations']*(1-0.5*scrho_l*rho_l['realisations']*scfy*fy['realisations']/(alpha_cc*scfc*fc['realisations']))*b*d**2
    ...
    alpha1, beta1 = FORM_tol(scfy,scrho_l, scfc,sceR,scGs,scGp,scQ,sceS,alpha_start=alpha0, beta_start=beta0)
    if MonteCarlo == True:
        beta1_mcs = MCS(scfy,scrho_l, scfc,sceR,scGs,scGp,scQ,sceS, 1e7)[0]
    
    
    
    
    if Print_results==True:
        print(u'\u03BC(fy) = {} MPa,\u03BC(rho_l) = {} , \u03BC(fc) = {} MPa ,\u03BC(eR) = {}'.format(scfy,scrho_l, scfc,sceR))     #write mu in unicode(\u03BC). {} acts as placeholder for the variables given in .format()
    ...
    #Printer heile iterasjonsprosessen Liljefors_input
    if Print_results==True:
        RES = FORM_tol(scfy,scrho_l, scfc,sceR,scGs,scGp,scQ,sceS,alpha_start=alpha0, beta_start=beta0, itreturn=True)
    ...
    #%%
    if B_plots==True:
        f1 = lambda scfy,scrho_l, scfc,sceR,scGs,scGp,scQ,sceS: FORM_tol(scfy,scrho_l, scfc,sceR,scGs,scGp,scQ,sceS,alpha_start=alpha0, beta_start=beta0)[1] #Only returns the beta-value
        #%%
        #fs/fc
