    ...
    def MCS(scfy,scrho_w,sceR,scGs,scGp,scQ,sceS,sceta,n_sim,alpha=alpha):
        ...
        if alpha==True:
            ...
            for i in range (len(a)): 
                p=[fy['realisations'],rho_w['realisations'],eR['realisations'],Gs['realisations'],Gp['realisations'],Q['realisations'],eS['realisations'],eta['realisations']]
                p[i]=p[i]*1.02
                gi=sceR*p[2]*scrho_w*p[1]*(1-sceta*p[7])*scfy*p[0]*bw*0.9*d*cot_theta-sceS*p[6]*(scGs*p[3]+scGp*p[4]+scQ*p[5])*(L/2-d) 
