    ...
    def MCS(scfy,scrho_l,scfc,sceR,scGs,scGp,scQ,sceSS,sceta,n_sim):
        ...
        if alpha==True:
            ...
            for i in range (len(a)): 
                ...
                p=[fy['realisations'],rho_l['realisations'],fc['realisations'],eR['realisations'],Gs['realisations'],Gp['realisations'],Q['realisations'],eS['realisations'],eta['realisations']]
                p[i]=p[i]*1.02#sc[i] #Endrer variabel i med d_i. Usikker på kor stor d_i bør være? 
                #Grensetilstand med endring i ein varabel:
                gi=sceR*p[3]*(scrho_l*p[1]*(1-sceta*p[8])*scfy*p[0]*(1-0.5*scrho_l*p[1]*(1-sceta*p[8])*scfy*p[0]/(alpha_cc*scfc*p[2]))*b*d**2)-sceS*p[7]*(scGs*p[4]+scGp*p[5]+scQ*p[6])*L**2/8
                failsi = np.zeros(n_sim);
                failsi[np.nonzero(gi<=0)[0]] = 1;
                #print(fy['realisations'][0])
        
        
                pfi=np.sum(failsi)/n_sim
        
                #Finner alfa for variabel i basert på endring i bruddsannsyn:
