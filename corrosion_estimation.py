...
def calc_p_ti(EX_class,tmax,n_sim,x=50):
    Fs=[] #Kumulativ fordeling
    pfs=[] #sannsynlighetstetthet
   
    
    betamc,pf=pf_MCS(EX_class,ec,D,x,1,n_sim) #x=50
    Fs+=[pf]
    pfs+=[pf]
    
    
    for t in range(2,tmax):
        betamc,pf=pf_MCS(EX_class,ec,D,x,t,n_sim) #Beregner sannsynlighet for initiert korrosjon ved kvart år frå t=0 til t=tmax.
...
#%% Finner pdf for initiering ved ulike overdekninger
cs=[40,50,60,80]
...
for XS in Ex:
    ...
    for c in cs:
        ...
        pfs=calc_p_ti(Ex[XS],100,1e6,x=c)[0]
        xt=fit_log_to_pdf(pfs,ts,[1,30],[(0,1),(0,200)])
        
        Ex[XS][str(c)]['pfs']=sp.stats.lognorm.pdf(ts,xt[0],loc=0,scale=xt[1])
    

#%% Plotter pdf for initiering ved ulike overdekninger
colors=['tab:red','tab:blue','g','y']
linestyles=['solid','dashed']
for i,c in enumerate(cs):
    #for n,XS in enumerate(Ex):
    #label=str(str(c)+'-'+str(XS))
    plt.plot(ts,Ex['XS1'][str(c)]['pfs'],color=colors[i],linestyle=linestyles[0])
    plt.plot(ts,Ex['XS3'][str(c)]['pfs'],color=colors[i],linestyle=linestyles[1],label=c)
...
#%% Beregner f(eta,t) for ulike overdekninger XS1
for c in cs:
        Ex['XS1'][str(c)]['eta_over_t']=calc_eta_over_t(Ex['XS1'],Ex['XS1'][str(c)]['pfs'],ts,r,1e6)
        
#%% Beregner f(eta,t) for ulike overdekninger XS3
for c in cs:
        Ex['XS3'][str(c)]['eta_over_t']=calc_eta_over_t(Ex['XS3'],Ex['XS3'][str(c)]['pfs'],ts,r,1e6)       
...
#%% Finner og lagrer parametre for eta over tid for XS1 for ulike overdekninger

for c in cs:
    m,s,p=find_params_eta(Ex['XS1'][str(c)]['eta_over_t'])
    Ex['XS1'][str(c)]['eta']={}
    Ex['XS1'][str(c)]['eta']['mean']=m
    #Ex['XS1']['eta']['n-mean']=nm
    Ex['XS1'][str(c)]['eta']['std']=s

    Ex['XS1'][str(c)]['eta']['params']=p
#%% Finner og lagrer parametre for eta over tid for XS1 for ulike overdekninger 
for c in cs:
    m,s,p=find_params_eta(Ex['XS3'][str(c)]['eta_over_t'])
    Ex['XS3'][str(c)]['eta']={}
    Ex['XS3'][str(c)]['eta']['mean']=m
    #Ex['XS1']['eta']['n-mean']=nm
    Ex['XS3'][str(c)]['eta']['std']=s

    Ex['XS3'][str(c)]['eta']['params']=p
    
    
#%% Plotter mean eta* for ulike overdekninger
colors=['tab:red','tab:blue','g','y']
linestyles=['solid','dashed']
plt.figure()
for i,c in enumerate(cs):
    plt.plot(ts[c:],Ex['XS1'][str(c)]['eta']['mean'][c:],color=colors[i],linestyle='solid') 

    plt.plot(ts[(c-30):],Ex['XS3'][str(c)]['eta']['mean'][(c-30):],color=colors[i],linestyle='dashed',label=c)
...
#%% Plotter V eta* for ulike overdekninger
colors=['tab:red','tab:blue','g','y']
linestyles=['solid','dashed']
plt.figure()
for i,c in enumerate(cs):
    plt.plot(ts[c:],Ex['XS1'][str(c)]['eta']['std'][c:]/Ex['XS1'][str(c)]['eta']['mean'][c:],color=colors[i],linestyle='solid') 

    plt.plot(ts[(c-30):],Ex['XS3'][str(c)]['eta']['std'][(c-30):]/Ex['XS3'][str(c)]['eta']['mean'][(c-30):],color=colors[i],linestyle='dashed',label=c)
...
def calc_alfas_eta(Beam,eta,t,bruddform='bending',tmin=0,MonteCarlo=False,alpha=False):
    alfas_eta=[]
    betas=[]
    betas_msc=[]
    alphas_msc=[]
    if bruddform=='bending':
        ...
        for i in range (tmin,len(t)):
            Beam['eta']=Bj.Create_dist_variable(eta['mean'][i], eta['std'][i]/(eta['mean'][i]), 'Lognormal') 
            if MonteCarlo==True:
                if alpha == True:
                    alfas,beta,beta_msc,alpha_msc=FORM_M_beam_eta(Beam,theta_R_bending,theta_E,alpha_cc=0.85,Print_results=False,B_plots=False,MonteCarlo=MonteCarlo,alpha=alpha)
                    betas_msc+=[beta_msc]
                    alphas_msc+=[alpha_msc]
                else:
                    alfas,beta,beta_msc=FORM_M_beam_eta(Beam,theta_R_bending,theta_E,alpha_cc=0.85,Print_results=False,B_plots=False,MonteCarlo=MonteCarlo,alpha=alpha)
                    betas_msc+=[beta_msc]
        ...
        for i in range (len(t)):
            ...
            if MonteCarlo==True:
                if alpha == True:
                    ...
                    alfas,beta,beta_msc=FORM_V_beam_eta(Beam,theta_R_shear,theta_E,alpha_cc=0.85,Print_results=False,B_plots=False,MonteCarlo=MonteCarlo,alpha=alpha)
                    betas_msc+=[beta_msc]
    ...
    if MonteCarlo==True:   
        if alpha == True:
            ...
            return np.array(alfas_eta),np.array(betas),np.array(betas_msc)
    else:
        return np.array(alfas_eta),np.array(betas)

#%% Berekner alfa og beta for bjelke M for t=40
BeamM['eta']=Bj.Create_dist_variable(Ex['XS1']['eta']['mean'][40], Ex['XS1']['eta']['std'][40]/(Ex['XS1']['eta']['mean'][40]), 'Lognormal') 
...
#%%
from FORM_M_beam_module import FORM_M_beam
#%% Tester alpha estimering for ukorrodert bjelke. Her virker det ikkje til å fungere veldig bra. Tar ikkje hensyn til V for dei ulike variablene
FORM_M_beam(BeamM,theta_R_bending,theta_E,alpha_cc=0.85,Print_results=True,B_plots=False,MonteCarlo=True)


#%% Berekner alfa og beta for bjelke M for t=[1,100]. På grunn av dårlig resultat frå FORM er MSC-beta mest viktig.
t=np.arange(1,100,1)

Ex['XS3']['eta']['Malfa'],Ex['XS3']['eta']['Mbeta'],Ex['XS3']['eta']['Mbeta-msc'],Ex['XS3']['eta']['Malfa-msc']=calc_alfas_eta(BeamM,Ex['XS3']['eta'],t,tmin=10,MonteCarlo=True,alpha=True)
#%%
Ex['XS1']['eta']['Malfa'],Ex['XS1']['eta']['Mbeta'],Ex['XS1']['eta']['Mbeta-msc'],Ex['XS1']['eta']['Malfa-msc']=calc_alfas_eta(BeamM,Ex['XS1']['eta'],t,MonteCarlo=True,alpha=True)


#%%  Berekner alfa og beta for bjelke V for t=[1,100] På grunn av dårlig resultat frå FORM er MSC-beta mest viktig.
Ex['XS3']['eta']['Valfa'],Ex['XS3']['eta']['Vbeta'],Ex['XS3']['eta']['Vbeta-msc'],Ex['XS3']['eta']['Valfa-msc']=calc_alfas_eta(BeamV,Ex['XS3']['eta'],t,bruddform='shear',MonteCarlo=True,alpha=True) #,Ex['XS3']['eta']['Valfa-msc']
#%%
Ex['XS1']['eta']['Valfa'],Ex['XS1']['eta']['Vbeta'],Ex['XS1']['eta']['Vbeta-msc'],Ex['XS1']['eta']['Valfa-msc']=calc_alfas_eta(BeamV,Ex['XS1']['eta'],t,bruddform='shear',MonteCarlo=True,alpha=True)
...
plt.plot(t[27:],-Ex['XS1']['eta']['Malfa-msc'][:,-1][27:],label='XS1') 
#plt.plot(t[10:],-Ex['XS3']['eta']['Malfa'][0:],label='XS3') 
plt.plot(t[10:],-Ex['XS3']['eta']['Malfa-msc'][:,-1][0:],label='XS3') 
...
plt.figure()
#for k,i in enumerate(Ex):
#plt.plot(t[27:],-Ex['XS1']['eta']['Valfa'][27:],label='XS1') #Alfa blir positiv siden høgare eta* gir lågare kapasitet. #!!!
plt.plot(t[27:],-Ex['XS1']['eta']['Valfa-msc'][:,-1][27:],label='XS1') 
#plt.plot(t[15:],-Ex['XS3']['eta']['Valfa'][15:],label='XS3') #Alfa blir positiv siden høgare eta* gir lågare kapasitet. #!!!
plt.plot(t[15:],-Ex['XS3']['eta']['Valfa-msc'][:,-1][15:],label='XS3')
...
plt.plot(t[15:],gamma_Rd_M_1/gamma_Rd_M_0*(1-calc_etad_star((Ex['XS3']['eta']['mean'][15:]),Ex['XS3']['eta']['std'][15:]/Ex['XS3']['eta']['mean'][15:],-0.1,B_t)),color='b',linestyle=line[1],label='XS3') 
...
plt.plot(t[15:],gamma_Rd_V_1/gamma_Rd_V_0*(1-calc_etad_star((Ex['XS3']['eta']['mean'][15:]),Ex['XS3']['eta']['std'][15:]/Ex['XS3']['eta']['mean'][15:],-0.1,B_t)),color='r',linestyle=line[1])  
...
#%% Plotter etad for ulike overdekninger
...
for i,c in enumerate(cs):
    ...
    plt.plot(t[c:],gamma_Rd_V_1/gamma_Rd_V_0*(1-calc_etad_star((Ex['XS1'][str(c)]['eta']['mean'][c:]),Ex['XS1'][str(c)]['eta']['std'][c:]/Ex['XS1'][str(c)]['eta']['mean'][c:],-0.28,B_t)),color=colors[i],linestyle='dotted')
    
    
    plt.plot(t[c:],gamma_Rd_M_1/gamma_Rd_M_0*(1-calc_etad_star((Ex['XS1'][str(c)]['eta']['mean'][c:]),Ex['XS1'][str(c)]['eta']['std'][c:]/Ex['XS1'][str(c)]['eta']['mean'][c:],-0.28,B_t)),color=colors[i],linestyle=linestyles[0],label=c)
    
    plt.plot(t[c:],(1-calc_etad_star((Ex['XS1'][str(c)]['eta']['mean'][c:]),Ex['XS1'][str(c)]['eta']['std'][c:]/Ex['XS1'][str(c)]['eta']['mean'][c:],-0.28,B_t)),color=colors[i],linestyle=(0,(3,5,1,5)))
...
plt.plot(t[6:],gamma_Rd_V_1/gamma_Rd_V_0*(1-calc_etad_star((Ex['XS3'][str(40)]['eta']['mean'][6:]),Ex['XS3'][str(40)]['eta']['std'][6:]/Ex['XS3'][str(40)]['eta']['mean'][6:],-0.28,B_t)),color=colors[0],linestyle='dotted')
plt.plot(t[6:],(1-calc_etad_star((Ex['XS3'][str(40)]['eta']['mean'][6:]),Ex['XS3'][str(40)]['eta']['std'][6:]/Ex['XS3'][str(40)]['eta']['mean'][6:],-0.28,B_t)),color=colors[0],linestyle=(0,(3,5,1,5)))
plt.plot(t[6:],gamma_Rd_M_1/gamma_Rd_M_0*(1-calc_etad_star((Ex['XS3'][str(40)]['eta']['mean'][6:]),Ex['XS3'][str(40)]['eta']['std'][6:]/Ex['XS3'][str(40)]['eta']['mean'][6:],-0.28,B_t)),color=colors[0],linestyle=linestyles[1])  


plt.plot(t[10:],gamma_Rd_V_1/gamma_Rd_V_0*(1-calc_etad_star((Ex['XS3'][str(50)]['eta']['mean'][10:]),Ex['XS3'][str(50)]['eta']['std'][10:]/Ex['XS3'][str(50)]['eta']['mean'][10:],-0.28,B_t)),color=colors[1],linestyle='dotted')
plt.plot(t[10:],(1-calc_etad_star((Ex['XS3'][str(50)]['eta']['mean'][10:]),Ex['XS3'][str(50)]['eta']['std'][10:]/Ex['XS3'][str(50)]['eta']['mean'][10:],-0.28,B_t)),color=colors[1],linestyle=(0,(3,5,1,5)))
plt.plot(t[10:],gamma_Rd_M_1/gamma_Rd_M_0*(1-calc_etad_star((Ex['XS3'][str(50)]['eta']['mean'][10:]),Ex['XS3'][str(50)]['eta']['std'][10:]/Ex['XS3'][str(50)]['eta']['mean'][10:],-0.28,B_t)),color=colors[1],linestyle=linestyles[1])  

plt.plot(t[15:],gamma_Rd_V_1/gamma_Rd_V_0*(1-calc_etad_star((Ex['XS3'][str(60)]['eta']['mean'][15:]),Ex['XS3'][str(60)]['eta']['std'][15:]/Ex['XS3'][str(60)]['eta']['mean'][15:],-0.28,B_t)),color=colors[2],linestyle='dotted')
plt.plot(t[15:],(1-calc_etad_star((Ex['XS3'][str(60)]['eta']['mean'][15:]),Ex['XS3'][str(60)]['eta']['std'][15:]/Ex['XS3'][str(60)]['eta']['mean'][15:],-0.28,B_t)),color=colors[2],linestyle=(0,(3,5,1,5)))
plt.plot(t[15:],gamma_Rd_M_1/gamma_Rd_M_0*(1-calc_etad_star((Ex['XS3'][str(60)]['eta']['mean'][15:]),Ex['XS3'][str(60)]['eta']['std'][15:]/Ex['XS3'][str(60)]['eta']['mean'][15:],-0.28,B_t)),color=colors[2],linestyle=linestyles[1])  

plt.plot(t[20:],gamma_Rd_V_1/gamma_Rd_V_0*(1-calc_etad_star((Ex['XS3'][str(80)]['eta']['mean'][20:]),Ex['XS3'][str(80)]['eta']['std'][20:]/Ex['XS3'][str(80)]['eta']['mean'][20:],-0.28,B_t)),color=colors[3],linestyle='dotted')
plt.plot(t[20:],(1-calc_etad_star((Ex['XS3'][str(80)]['eta']['mean'][20:]),Ex['XS3'][str(80)]['eta']['std'][20:]/Ex['XS3'][str(80)]['eta']['mean'][20:],-0.28,B_t)),color=colors[3],linestyle=(0,(3,5,1,5)))
plt.plot(t[20:],gamma_Rd_M_1/gamma_Rd_M_0*(1-calc_etad_star((Ex['XS3'][str(80)]['eta']['mean'][20:]),Ex['XS3'][str(80)]['eta']['std'][20:]/Ex['XS3'][str(80)]['eta']['mean'][20:],-0.28,B_t)),color=colors[3],linestyle=linestyles[1])  
...
plt.plot(t[15:],Ex['XS3']['eta']['Mbeta'][5:],label='XS3-FORM')
plt.plot(t[27:],Ex['XS1']['eta']['Mbeta-msc'][27:],label='XS1-msc')
plt.plot(t[15:],Ex['XS3']['eta']['Mbeta-msc'][5:],label='XS3-msc')
#plt.axhline(-0.7,color='g')
plt.xlabel('t [y]')
plt.ylabel(r'$\beta$')
#plt.ylim([0,1])
...
Ex['XS1']['etad']=1-calc_etad_star((Ex['XS1']['eta']['mean'][27:]),Ex['XS1']['eta']['std'][27:]/Ex['XS1']['eta']['mean'][27:],-0.28,B_t)
Ex['XS3']['etad']=1-calc_etad_star((Ex['XS3']['eta']['mean'][15:]),Ex['XS3']['eta']['std'][15:]/Ex['XS3']['eta']['mean'][15:],-0.28,B_t)
...
plot_UB(ts[27:],UTNs_V_XS1,Ex['XS1']['eta']['Vbeta-msc'][27:],70)

plot_UB(ts[15:],UTNs_V_XS3,Ex['XS3']['eta']['Vbeta-msc'][15:],16)

#%% Plotter UTN og Beta_msc for beam M
plot_UB(ts[27:],UTNs_M_XS1,Ex['XS1']['eta']['Mbeta-msc'][27:],70)

plot_UB(ts[15:],UTNs_M_XS3,Ex['XS3']['eta']['Mbeta-msc'][5:],16)

#%% Finner etad med mcs-estimert alpha for bjelke M

Ex['XS1']['M-etad-mcs']=1-calc_etad_star((Ex['XS1']['eta']['mean'][27:]),Ex['XS1']['eta']['std'][27:]/Ex['XS1']['eta']['mean'][27:],-Ex['XS1']['eta']['Malfa-msc'][:,-1][27:],B_t)
Ex['XS3']['M-etad-mcs']=1-calc_etad_star((Ex['XS3']['eta']['mean'][15:]),Ex['XS3']['eta']['std'][15:]/Ex['XS3']['eta']['mean'][15:],-Ex['XS3']['eta']['Malfa-msc'][:,-1][5:],B_t)
#%%
UTNs_M_XS1_MCS=np.array(list(map(lambda etad: Controll_psf_beam_etad(BeamM,etad,alpha_cc=0.85,Bending=True,Shear=False,Print_results=False,corrosion=True),Ex['XS1']['M-etad-mcs'])))
UTNs_M_XS3_MCS=np.array(list(map(lambda etad: Controll_psf_beam_etad(BeamM,etad,alpha_cc=0.85,Bending=True,Shear=False,Print_results=False,corrosion=True),Ex['XS3']['M-etad-mcs'])))


#%%  Beregner og lotter utnyttelse med semiprobabilistisk kontroll
ts=np.arange(1,100,1 )
plot_UB(ts[27:],UTNs_M_XS1_MCS,Ex['XS1']['eta']['Mbeta-msc'][27:],80)

plot_UB(ts[15:],UTNs_M_XS3_MCS,Ex['XS3']['eta']['Mbeta-msc'][5:],17)

#%% Finner etad med mcs-estimert alpha for bjelke V

Ex['XS1']['V-etad-mcs']=1-calc_etad_star((Ex['XS1']['eta']['mean'][27:]),Ex['XS1']['eta']['std'][27:]/Ex['XS1']['eta']['mean'][27:],-Ex['XS1']['eta']['Valfa-msc'][:,-1][27:],B_t)
Ex['XS3']['V-etad-mcs']=1-calc_etad_star((Ex['XS3']['eta']['mean'][15:]),Ex['XS3']['eta']['std'][15:]/Ex['XS3']['eta']['mean'][15:],-Ex['XS3']['eta']['Valfa-msc'][:,-1][15:],B_t)
#%% Beregner og lotter utnyttelse med semiprobabilistisk kontroll
UTNs_V_XS1_MCS=np.array(list(map(lambda etad: Controll_psf_beam_etad(BeamM,etad,alpha_cc=0.85,Bending=True,Shear=False,Print_results=False,corrosion=True),Ex['XS1']['V-etad-mcs'])))
UTNs_V_XS3_MCS=np.array(list(map(lambda etad: Controll_psf_beam_etad(BeamM,etad,alpha_cc=0.85,Bending=True,Shear=False,Print_results=False,corrosion=True),Ex['XS3']['V-etad-mcs'])))


plot_UB(ts[27:],UTNs_V_XS1_MCS,Ex['XS1']['eta']['Vbeta-msc'][27:],61)

plot_UB(ts[15:],UTNs_V_XS3_MCS,Ex['XS3']['eta']['Vbeta-msc'][15:],14)
