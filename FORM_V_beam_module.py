...
def FORM_V_beam(Beam,theta_R_M,theta_E,alpha_cc=0.85, Print_results=False, MonteCarlo=False,B_plots=False,SCrho=1):
...
#%%  #The limit state as a function of the scaling factors:
    gX = lambda x,scfy,scrho_w,sceR,scGs,scGp,scQ,sceS: sceR*x[2]*scrho_w*x[1]*scfy*x[0]*bw*0.9*d*cot_theta-sceS*x[6]*(scGs*x[3]+scGp*x[4]+scQ*x[5])*(L/2-d)
    ...
    U = [x2u(fy),x2u(rho_w),x2u(eR),x2u(Gs),x2u(Gp),x2u(Q),x2u(eS)] 
    
    #%% We can now write our LSF as a function of u:
    
    
    #LSF as a function of u, scX:
    gU = lambda u,scfy,scrho_w,sceR,scGs,scGp,scQ,sceS: gX([U[0](u[0]),
                                          U[1](u[1]),
                                          U[2](u[2]),U[3](u[3]),U[4](u[4]),U[5](u[5]),U[6](u[6])],
                                                         scfy,scrho_w,sceR,scGs,scGp,scQ,sceS) 
    ...
    def alpha_next(u,scfy,scrho_w,sceR,scGs,scGp,scQ,sceS):
        dgdu = np.zeros(7)
        
        #Resistance
        dgdu[0] = sceR*U[2](u[2])*scrho_w*U[1](u[1])*scfy*dxdufun(fy)(u[0])*bw*0.9*d*cot_theta #fy
        dgdu[1] = sceR*U[2](u[2])*scrho_w*dxdufun(rho_w)(u[1])*scfy*U[0](u[0])*bw*0.9*d*cot_theta #rho_l
        dgdu[2] = sceR*dxdufun(eR)(u[2])*scrho_w*U[1](u[1])*scfy*U[0](u[0])*bw*0.9*d*cot_theta #eR
    ...
    alpha0 = np.array([-1/np.sqrt(7),-1/np.sqrt(7),-1/np.sqrt(7),1/np.sqrt(7),1/np.sqrt(7),1/np.sqrt(7),1/np.sqrt(7)])
   ...
   #Løyser g(u)=0 med hensynn til beta, basert på forrige beta og gamma
    def beta_next(alpha, beta_prev,scfy,scrho_w,sceR,scGs,scGp,scQ,sceS):
        equation = lambda beta: gU(beta*alpha,scfy,scrho_w,sceR,scGs,scGp,scQ,sceS) #this defines the fsolve should work with.
    ...
    def FORM_tol(scfy,scrho_w,sceR,scGs,scGp,scQ,sceS,alpha_start, beta_start, tol=1e-5, itreturn=False):
        alphas = [alpha_start]
        beta1 = beta_next(alpha_start, beta_start, scfy,scrho_w,sceR,scGs,scGp,scQ,sceS)
        ...
        while abs((betas[-1] - betas[-2])/betas[-1]) >= tol:
            alpha_new = alpha_next(betas[-1]*alphas[-1],scfy,scrho_w,sceR,scGs,scGp,scQ,sceS) #calculates new, improved alpha, using the last calculated values (by using [-1])
            alphas.append(alpha_new)                         #adds the new, improved alpha to the list of alphas
            beta_new = beta_next(alphas[-1],betas[-1],scfy,scrho_w,sceR,scGs,scGp,scQ,sceS)   #calculates new, improved beta
   ...
   #Monte Carlo simulation 
    ...
    def MCS(scfy,scrho_w,sceR,scGs,scGp,scQ,sceS,n_sim):
        ...
        # LSF for MCS:
        g=sceR*eR['realisations']*scrho_w*rho_w['realisations']*scfy*fy['realisations']*bw*0.9*d*cot_theta-sceS*eS['realisations']*(scGs*Gs['realisations']+scGp*Gp['realisations']+scQ*Q['realisations'])*(L/2-d)
    ...
    alpha1, beta1 = FORM_tol(scfy,scrho_w,sceR,scGs,scGp,scQ,sceS,alpha_start=alpha0, beta_start=beta0)
    if MonteCarlo == True:
        beta1_mcs = MCS(scfy,scrho_w,sceR,scGs,scGp,scQ,sceS, 1e7)[0]
    
    
    
    
    if Print_results ==True:
        print(u'\u03BC(fy) = {} MPa,\u03BC(rho_w) = {}  ,\u03BC(eR) = {}'.format(scfy,scrho_w,sceR))     #write mu in unicode(\u03BC). {} acts as placeholder for the variables given in .format()
    ...
    if Print_results ==True:
        RES = FORM_tol(scfy,scrho_w,sceR,scGs,scGp,scQ,sceS,alpha_start=alpha0, beta_start=beta0, itreturn=True)
